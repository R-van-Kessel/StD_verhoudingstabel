...
function generateQuestions() {
    ...
    // Verschillende soorten verhoudingstabellen
    let tableTypes = [
        { unit1: "kg", unit2: "€" },
        { unit1: "L", unit2: "€" },
        { unit1: "st", unit2: "€" },
    ...
    for (let i = 0; i < 10; i++) {
        let type = random(tableTypes);
        
        // Genereer basisverhouding
        let val1a = floor(random(2, 20)); // Eerste kolom, eerste rij
        let val2a = floor(random(5, 50)); // Eerste kolom, tweede rij
        
        // Vermenigvuldigingsfactor voor tweede kolom
        let multiplier = floor(random(2, 8));
        let val1b = val1a * multiplier;
        let val2b = val2a * multiplier;
        
        // Kies willekeurig welk vakje ontbreekt
        let missingPosition = floor(random(4));
        let answer;
        
        let displayVal1a, displayVal1b, displayVal2a, displayVal2b;
        
        // In de generateQuestions functie, vervang dit deel:

if (missingPosition === 0) {
    answer = val1a;
    displayVal1a = "?";
    displayVal1b = val1b + type.unit1;
    displayVal2a = val2a + type.unit2;
    displayVal2b = val2b + type.unit2;
} else if (missingPosition === 1) {
    answer = val2a;
    displayVal1a = val1a + type.unit1;
    displayVal1b = val1b + type.unit1;
    displayVal2a = "?";
    displayVal2b = val2b + type.unit2;
...
// Horizontale tabel met eenheden
let text = displayVal1a + " " + displayVal1b + "\n" + 
           displayVal2a + " " + displayVal2b;
    ...
    for (let row = 1; row < 3; row++) {
        for (let col = 0; col < COLS; col++) {
            blocks.push({
                ...
                hoverProgress: 0, 
                isRatioTable: true // Markeer als verhoudingstabel voor kleinere tekst
            });
            questionIndex++;
        }
    }
...
function drawBlock(block) {
  ...
  // ====== VERHOUDINGSTABEL LIJNEN (altijd zichtbaar) ======
  if (block.isQuestion) {
    stroke(0, 0, 0);
    strokeWeight(2);
    // Verticale middenlijn
    line(bx + CELL_SIZE / 2, by + 5, bx + CELL_SIZE / 2, by + CELL_SIZE - 5);
    // Horizontale middenlijn
    line(bx + 5, by + CELL_SIZE / 2, bx + CELL_SIZE - 5, by + CELL_SIZE / 2);
  }
  ...
  if (block.isQuestion) {
    ...
    let topLine    = lines[0] ? lines[0].split(' ') : ['', ''];
    let bottomLine = lines[1] ? lines[1].split(' ') : ['', ''];

    let qx = CELL_SIZE / 4;
    let qy = CELL_SIZE / 4;

    // Linksboven
    text(topLine[0]    || '', bx + qx,     by + qy);
    // Rechtsboven
    text(topLine[1]    || '', bx + qx * 3, by + qy);
    // Linksonder
    text(bottomLine[0] || '', bx + qx,     by + qy * 3);
    // Rechtsonder
    text(bottomLine[1] || '', bx + qx * 3, by + qy * 3);
  } else {

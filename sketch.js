...
function showInfo() {
    ...
    popup.innerHTML = `
        ...
        <p style="color: #0E0E0E; line-height: 1.2;">
            ...
            <strong>Hoe speel je:</strong>self
...
function drawBlock(block) {
    ...
    if (block.isRatioTable) {
        ...
        textSize(16); // Normaal lettertype voor antwoorden
    }
    
    // Teken tekst (met multi-line ondersteuning voor verhoudingstabellen)
    if (block.text.includes('\n')) {
        let lines = block.text.split('\n');
        
        if (block.isRatioTable) {
            // Voor verhoudingstabellen: splits per regel en per kolom
            let row1Parts = lines[0].split(' '); // Bijvoorbeeld: ["3kg", "12kg"]
            let row2Parts = lines[1].split(' '); // Bijvoorbeeld: ["15€", "?€"]
